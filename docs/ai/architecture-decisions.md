...
- Consequence: Cards and the admin form now reflect the real variant cover while keeping one batched query per listing (no N+1 per product). The homepage `backfillVariantPrimaryImages` workaround became redundant and was removed. `mapProductToCard`/`mapProductImages` and the form mapping are unchanged because they consume the already-merged `images` array.