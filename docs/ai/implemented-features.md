...
- Storefront category navbar is a full-width pill carousel (`AppNavbar` server → `StorefrontNavbarCarousel` client) rendered below the header's dark brand strip: categories appear as circular image pills sorted alphabetically with a trailing `All Categories` tile to `/categories`; mobile shows exactly five pills per view with arrow icons while `md`+ uses natural widths (arrows only on overflow); each category title opens a scrollable product dropdown (lazy `GET /api/catalog/categories/[slug]/products` capped at 8, memoized per category) on hover or click but never during a carousel drag; category images use small responsive `sizes` (no `vw`), and the header dark background is confined to the upper logo/actions strip so the navbar sits on the plain page background
...
- Saved addresses (2026-08-31): signed-in customers can save a delivery address directly from checkout via a **Save** button in the Shipping address card (tooltip: "Save the address to use for later orders, You can change your address from addresses inside profile") plus a **Manage saved addresses** link to `/account/addresses`; the checkout save upserts by street1 so re-saving never duplicates. The profile **Addresses** page (`/account/addresses`) is a full client address book (`AddressBook` + `AddressForm` in a dialog) with Add / Edit / Remove / Make-default actions, backed by the existing Prisma `Address` model and guarded API routes `GET|POST /api/addresses` + `PATCH|DELETE /api/addresses/[addressId]` (the PATCH handler accepts either a full address payload or `{ isDefault: true }`). Service lives in `src/features/addresses/` (`listSavedAddresses`, `getSavedAddress`, `upsertSavedAddress` — first saved address auto-defaults, `updateSavedAddress`, `setDefaultSavedAddress`, `deleteSavedAddress`); city/province/country are locked to Karachi / Sindh / Pakistan and the optional `phone` field accepts an empty string (normalized to `undefined`). The save controls render only for authenticated customers.
