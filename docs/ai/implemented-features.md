...
- Variant cover images on listing cards: storefront listing queries (`listPublishedProductsByCategory`, `listAllPublishedProducts`, `getRelatedPublishedProducts`, `listPublishedProductsByIds`, `searchPublishedProducts`) batch-fetch variant-level images and merge them into each product record, so category grids, search results, related-product cards, and homepage sections show the real cover for variant-only media instead of the placeholder gradient
