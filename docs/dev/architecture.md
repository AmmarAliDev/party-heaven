...
- The sticky header (`HeaderScrollHide`) is split into two visually distinct bands so the navbar is never painted with the dark brand background: the dark strip (`bg-background-header-footer`) wraps only the upper row (logo + actions), and the category navbar renders below it on its own full-width `bg-background` band. Both bands hide together on scroll-down.
- Storefront category navigation is a full-width pill bar (`src/components/layout/app-navbar.tsx`, server → `src/components/layout/storefront-navbar-carousel.tsx`, client) that renders live published catalog categories as circular image pills inside an embla carousel. Categories are sorted alphabetically by `buildStorefrontNavbarCategories` (`src/components/layout/storefront-navbar-categories.ts`), which always appends an `All Categories` tile linking to `/categories`. Layout: `basis-1/5` on mobile (exactly five pills per view with arrow icons) and natural (`basis-auto`) widths from the `md` breakpoint up, so arrows appear only on overflow. The circular image links to the category page; the title underneath opens a dropdown of that category's products, lazy-loaded from the existing `GET /api/catalog/categories/[slug]/products` endpoint capped at `NAVBAR_CATEGORY_DROPDOWN_PRODUCT_LIMIT` (8) and memoized per slug. Because the cap is small the panel grows to its natural height (no inner scrollbar); on mobile it is a wide, horizontally screen-centred panel (`w-[80vw]`, kept centred with an rAF effect that nudges the Radix popper wrapper back to the viewport centre), while on `md`+ it stays a fixed-width card anchored under the title. Carousel drags never open the dropdowns (drag detection + hover/click guards in the carousel). The navbar also auto-slides (embla-carousel-autoplay, 4s) whenever the row is scrollable and the visitor is idle: auto-slide pauses on hover, during a drag, and while a product dropdown is open, and resumes after the pointer leaves / the dropdown closes. Autoplay is restarted from embla's `reInit`/`resize`/`slidesChanged` handlers because embla re-creates plugins when the slide layout changes (e.g. category images load).
- `Home`, `About`, `Blog`, and `Contact` no longer render as standalone desktop navbar links; they live inside the shared `<UserMenu />` dropdown (`StorefrontHeaderAuthControls` → `UserMenu`), which includes `Your Orders` (`/account/orders`) before `Sign out` for signed-in users. On mobile there is no separate user-menu button — the hamburger drawer (`StorefrontMobileNav`) hosts the full menu: page links, `Admin Panel` (admins), `Your Orders` (signed-in users), and wishlist/account/sign-out actions. The drawer no longer lists catalog categories; category browsing on mobile is served by the mobile bottom nav **Collections** action.
- The category menu contract is stable: published categories are sorted alphabetically in the navbar, and `All Categories` is always the final navbar tile (links to `/categories`), preserving SEO-friendly listing discoverability. The storefront footer reuses the `More`-style split (`buildStorefrontNavbarCategoryMenu`, capped by `NAVBAR_DIRECT_CATEGORY_LIMIT`) for its own text-only `Quick Links` column.
- Header/footer category load failures are non-fatal: errors are logged server-side; the navbar hides itself entirely (`AppNavbar` returns `null` when categories fail or are empty) and the footer falls back to an empty `Quick Links` column.
...
- Party Heaven category count for shared storefront surfaces now uses `countPublishedPartyHeavenProducts` (single-variant-per-product read) instead of loading full product payloads.
- `countPublishedPartyHeavenProducts` now executes the count in SQL (LATERAL selected variant: default first, then oldest fallback) and returns only a scalar, avoiding large JS-side payload scans on `/categories` and header renders.
- The Party Heaven count query keeps a guarded Prisma fallback path if raw SQL fails in constrained runtimes, preserving storefront correctness over hard failure.
...
- `Party Heaven` is implemented as a virtual/system storefront category (`slug: party-heaven`) in `src/features/catalog/party-heaven.ts`; it is not persisted as a `Category` row and does not alter product-to-category relationships.
- Party Heaven membership is derived at read-time from published products: include when default selling price is `<= Rs. 280`; products stay assigned to their original categories while also appearing in this special listing.
- The `party-heaven` slug is reserved for the virtual category. If a published DB category collides with this slug, storefront category surfaces suppress the physical duplicate and log a warning for operator follow-up.
...
- Product cards (`ProductGridCard`, `ProductRelatedGrid`, homepage `featured-products`/`party-heaven` carousels) render `ProductCardAddToCart` as an absolutely positioned sibling (`z-10`) of the card's wrapping `Link`, so the card remains a single link while the button is independently clickable; clicking it posts to `POST /api/cart` (quantity 1, product slug) and opens the drawer. Homepage cards render the button only when the product has a slug (fallback/placeholder items have none).
