...
- Detail-page image loading: Prisma's `Product.images` relation only returns product-level rows (`productId` set). The detail query (`_getPublishedProductBySlugImpl` in `src/server/db/catalog-queries.ts`) additionally fetches variant-level images (`productVariantId` set) scoped to the product's variants and merges them into the record so the PDP gallery can render variant-specific media.
