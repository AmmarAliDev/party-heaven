...
- The category navbar (`src/components/layout/app-navbar.tsx` server → `src/components/layout/storefront-navbar-carousel.tsx` client) renders live catalog categories as circular image pills inside an embla carousel where **every category is an equal-width slot at every breakpoint** (`basis-1/4 sm:basis-1/5 md:basis-1/6 lg:basis-1/7 xl:basis-1/8 2xl:basis-1/9` — never `basis-auto`), so the distance between categories is always the same and the row scrolls/auto-slides to reveal categories that don't fit. Category titles sit under the circle inside a `line-clamp-2` (`text-wrap`, no `truncate`) so a name wraps to a second line instead of being cut off on smaller screens; the row stretches so every pill is the same height and the circles stay aligned. The circular image links to the category page; the title underneath opens a dropdown of that category's products (lazy-loaded, capped at `NAVBAR_CATEGORY_DROPDOWN_PRODUCT_LIMIT` = 8, memoized per category). The panel shows all fetched products at natural height (no inner scrollbar); on mobile it is 80% of the viewport width and horizontally centred on the screen, and on `md`+ it is a fixed-width (`w-72`) card anchored under the title. Carousel drags never open the dropdowns. The row auto-slides (`embla-carousel-autoplay`, 4s delay) only while it is scrollable and idle; auto-slide pauses on hover, drag, or an open dropdown and resumes when the visitor stops interacting.
- Category art uses small, responsive `next/image` `sizes` per breakpoint (no `vw` unit) because it only ever renders inside a small circle; small intrinsic dimensions + `sizes` keep the requested image tiny.
- The header's upper row (logo + actions) is the only section painted with the dark `bg-background-header-footer`; the category navbar sits on its own full-width `bg-background` band below (`HeaderScrollHide` is `flex-col`), so the two regions stay individually visible and the navbar never carries a dark background.
- The header's top row is responsive: on mobile it is a three-zone layout — hamburger drawer trigger on the far left, centered logo, and right-side actions (search trigger — intentionally hidden for now —, cart rightmost; no separate user-menu button — the hamburger drawer hosts the full menu); on desktop the logo sits on the leading edge and the right side holds search, wishlist, cart drawer, and the user menu. The mobile logo is narrower (`h-8 w-40 md:h-10 md:w-60 lg:h-20 lg:w-70`) so the centered layout fits small screens without overflow, and the header row uses equal side columns (`grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)]`) so the logo stays truly centered.
...
- The mobile drawer does not render catalog categories; the full-width category pill carousel is visible on every viewport (including mobile), and the bottom nav's **Collections** action still links to `/categories`.
- Category ordering is deterministic: published categories render alphabetically, and an `All Categories` tile is always the final navbar item (links to `/categories`).
